...
class LabelInfo:
    ...
    def _get_properties(cls, properties_def, properties):
        ...
        for prop in properties:
            ...
            if prop_def["type"] in ["radio", "dropdown", "checkbox"]:
                converted_properties.append(
                    {"name": prop["propertyName"], "value": prop["optionName"]}
                )
            elif prop_def["type"] == "free response":
                converted_properties.append(
                    {
        ...
        self.result["objects"].append(
            {
                ...
                "annotationType": self.object_classes_map[class_name][
                    "annotation_type"
                ],
                "frames": [
                    {
                    ...
                    "annotations": [
                        {
                            ...
                            "properties": LabelInfo._get_properties(
                                self.object_classes_map[obj["className"]][
                                    "properties"
                                ],
                                frame["properties"],
                            ),
    ...
    def build_tags(self):
        ...
        classes_name = {}
        for obj in self.result["objects"]:
            classes_count[obj["classId"]] = (
                classes_count.get(obj["classId"], 0) + 1
            )
            anno_count[obj["classId"]] = anno_count.get(
                obj["classId"], 0
            ) + len(obj["frames"])
            ...
            "meta": {
                ...
                "editInfo": {
                    "objects": [
                        {
                            "id": obj["id"],
                            "color": self.object_classes_map[obj["className"]][
                                "color"
                            ],
                            "visible": True,
                            "selected": False,
