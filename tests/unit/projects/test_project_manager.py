...
from .mocks import MOCK_PROJECT, MOCK_PROJECT_RESPONSE_JSON, MOCK_PROJECTS_RESPONSE_JSON
...
class ProjectManagerTest(unittest.TestCase):
    ...
    def test_get_project_query_with_right_name(self):
        ...
        self.assertEqual(
            query,
            "query ($name:String) {projects(name:$name,page:1,pageSize:1){count, edges{id name labelInterface workapp settings isPublic createdAt createdBy lastUpdatedAt lastUpdatedBy progress submittedLabelCount inProgressLabelCount skippedLabelCount stats}}}",
        )
    ...
    def test_get_project_list_query(self):
        ...
        self.assertEqual(
            query,
            "query  {projects(page:1,pageSize:10){count, edges{id name labelInterface workapp settings isPublic createdAt createdBy lastUpdatedAt lastUpdatedBy progress submittedLabelCount inProgressLabelCount skippedLabelCount stats}}}",
        )
